...
int main(int argc, char **argv) {
    ...
    // TEST initialisiere char array result mit staetten
    char hilfe[MAX_LAENGE_STR][MAX_LAENGE_STR];
    int j = -1;
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        {
            j = j + 1;
            sprintf(hilfe[j], "Die Stadt %s hat %d Einwohner.", staedte[i], bewohner[i]);
        }
    }

    // Speicher alloziieren
    char **result;
    result = malloc((sizeof(char*)*(j+1)*100));
